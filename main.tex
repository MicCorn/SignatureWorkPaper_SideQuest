...
% Numeric citations are common in CS. This is also the "least hassle" with biblatex.
...
\instructions{There is a very long background text here that is required. I will paste it in later}

% Author name (capitalized in its regular way)
\newcommand{\authorname}{Michael Cornell}

% Title (cannot exceed three lines)
\newcommand{\thetitle}{SW PROJECT TITLE THAT EXTENDS OVER ONE LINE GOES IN INVERTED PYRAMID FORM}
...
\instructions{Individuals and organizations who helped with the project and provided financing are thanked here. Do not include individual titles. You may include grant numbers and sponsors (SELF, SRS, SW Grants, etc.).}
\newpage
...
\section{Problem Statement}
 Modern social media platforms often encourage users to passively consume content instead of interacting with their environment. Passive consumption leads to sedintary lifestyles, dissasociates users from the environment they live in, and doesn't spark connection. Additionally, exisiting platforms like Google Maps reduce meaningful locations to simple pins. This project addresses that gap by designing a social platform that encourages users to explore locations and participate in challenges tied to real-world places.
...
% Example inline figure (placeholder)
\begin{figure}[htbp]
\centering
\includegraphics[height=4cm]{btc.jpg}
\caption{Placeholder figure. Replace with an early concept sketch, moodboard, or UI screenshot.}
...
SideQuest was inspired by elements of many different apps/experiences. Some of the ideas that inspired us to create SideQuest go back to the 1990's, whereas some are still in development today. To better understand SideQuest, it is important to understand existing location based media, online social networks (OSNs), and gameification. 

\section{Location-Based Media}
Location-based media represent a category of digital systems that integrate computational experiences with physical movement through geographic space. Early research in this field explored how mobile technologies could extend digital interaction beyond desktop environments and into the streets, parks, and public spaces of cities.

\textbf{Case Study:} Can You See Me Now? (CYSMN)
The mixed-reality game Can You See Me Now (CYSMN) is one of the earliest examples of a location-based media, combining online and mobile players within a shared urban environment. Participants physically navigated city streets while interacting with digital players through location-aware devices. Studies of CYSMN highlight how location-based experiences can blend digital and physical environments to produce new forms of interaction embedded within everyday spaces. At the same time, the deployment of such systems revealed technical and experiential challenges, including uncertainty in positioning data, unreliable connectivity, and the difficulty of managing complex interactions in uncontrolled real-world environments \cite{CYSMN}.  <<-- rework this

\textbf{Case Study:} Geocaching
As mobile technologies matured, location-based interaction moved beyond experimental installations and toward large-scale community systems. One of the earliest and most successful examples is Geocaching, a global activity in which participants hide and discover containers at specific GPS coordinates. Research on geocaching emphasizes the importance of community participation in sustaining location-based systems. Players not only participate in the game but also create and maintain its content, placing caches and documenting discoveries. This model demonstrates how user-generated contributions can enable location-based platforms to scale while maintaining long-term engagement. \cite{Geocaching}

Other experimental systems explored how gameplay could generate useful geographic information. In the mobile game EyeSpy, players tagged locations with photographs and textual descriptions while exploring urban environments. Other participants could verify these tags by visiting the same locations, earning points while confirming their accuracy. As a result, the game produced a collection of geo-referenced images and descriptions that could later support navigation tasks. This work illustrates how playful interaction can produce valuable datasets through what researchers describe as “human computation,” where users collectively generate information through game-like activities. \cite{Eyespy}

Today, location-based interaction has become widespread through mapping platforms such as Google Maps and Apple Maps. These systems structure how individuals navigate cities and choose destinations, often incorporating user-generated reviews and ratings that influence travel decisions. However, while mapping platforms provide rich geographic information, they typically function as informational tools rather than interactive experiences.

A major shift occurred with the release of Pokémon GO, which demonstrated the commercial potential of location-based augmented reality games. Pokémon GO overlays a fictional game world onto real geographic locations, requiring players to physically move through urban environments in order to progress in the game. Studies of player experiences show that the game encourages outdoor movement, social encounters, and engagement with public spaces. By leveraging widely available smartphone technologies, Pokémon GO became the first location-based augmented reality game to achieve mainstream global adoption. \cite{Pokemon}

Beyond encouraging physical movement, research also suggests that Pokémon GO fosters social interaction and community engagement. Players frequently encounter others while visiting shared in-game locations, and these interactions can contribute to a sense of belonging connected to specific places. In this way, location-based gameplay can facilitate both exploration and social connection within real-world environments. \cite{PokemonBelong}

Despite these advances, most location-based systems maintain a largely one-directional relationship between digital and physical space. Real-world movement influences digital experiences, but digital systems rarely generate new forms of interaction within physical environments beyond predefined gameplay mechanics.

\section{Social Media and Spontaneity}
While location-based systems emphasize movement and spatial exploration, social media platforms shape how individuals share and interpret experiences online. However, the dominant design of many contemporary social platforms has increasingly shifted toward passive content consumption.

Research on viral marketing and online interaction suggests that engagement with digital content typically follows a two-stage decision process. Users must first decide whether to pay attention to content and then determine whether to interact with it through actions such as sharing, commenting, or responding. Many modern platforms optimize for maximizing the number of views rather than encouraging active participation, resulting in ecosystems where a relatively small percentage of users create content while the majority primarily consume it. \cite{Viral}

In response to this trend, some newer platforms have experimented with mechanisms that encourage more spontaneous forms of participation. BeReal represents a prominent example of this shift. The application prompts users once per day to capture and share a photograph within a short time window, simultaneously recording both the user and their surrounding environment \cite{BeReal}. This format reduces the opportunity for curation or editing and instead emphasizes authenticity and everyday moments.

Researchers have described platforms such as BeReal as part of a broader category of spontaneous online social networks (SOSNs). These systems differ from traditional social media by prioritizing real-time participation and authenticity over carefully curated content. Emerging research suggests that such platforms may influence how individuals share experiences during travel, encouraging more immediate and unfiltered forms of documentation. \cite{BerealTourism}

Some experimental social platforms extend this idea by incorporating structured prompts or challenges that guide user participation. The application Hyper, for example, encourages users to complete themed photo or video challenges throughout their daily routines. By framing everyday activities as playful tasks and providing points or rewards for participation, the platform transforms ordinary moments into structured social interactions. The system also uses automated verification to determine whether submitted content satisfies challenge criteria. \cite{Hyper}

Together, these developments suggest that structured prompts and participation mechanics can play an important role in shifting users from passive consumption toward active engagement.

\section{Gamification Theory}
Gamification refers to the application of game design elements—such as rewards, progression systems, and competition—to non-game contexts in order to motivate user behavior. These techniques have been widely adopted across digital platforms seeking to encourage sustained engagement.

Location-based games such as Pokémon GO illustrate how spatial movement can become a central gameplay mechanic. By distributing objectives across real-world locations, the game incentivizes players to walk through cities and explore new environments. Studies of the Pokémon GO experience highlight how such mechanics can encourage both physical activity and social interaction among participants. \cite{Pokemon}

Gamification has also been explored within tourism contexts, where researchers have examined how location-based activities can enhance visitor engagement with destinations. Geocaching, for example, has been proposed as a model for gamified tourism experiences that encourage travelers to explore locations in playful ways. Research suggests that younger generations in particular may seek digitally mediated experiences that combine exploration with interactive gameplay. \cite{GeoTourism}

More broadly, gamified systems rely on feedback loops that reward participation and encourage repeated engagement. Progress indicators, points, streaks, and challenges can transform routine activities into experiences that feel more playful and motivating. These mechanics demonstrate how digital systems can structure participation in ways that influence both behavior and perception.

\section{Social Media and Tourism}
Digital platforms increasingly shape how individuals discover and evaluate travel destinations. Historically, travelers relied on guidebooks, blogs, or travel agencies to identify locations worth visiting. Today, much of this information is mediated through digital mapping platforms and online reviews, which play a central role in shaping travel decisions.

At the same time, social media platforms influence tourism by encouraging individuals to document and share their experiences. Recent research suggests that spontaneous social networks such as BeReal may further reshape tourism behaviors by encouraging travelers to capture authentic moments rather than carefully staged travel imagery. These dynamics may influence not only how people document travel but also how they decide where to go and what experiences to pursue. \cite{BerealTourism}

This shift reflects a broader cultural trend toward experiences that are both personally meaningful and socially shareable.

\section{Synthesis: Design Opportunity}
Across these areas of research, several patterns emerge. Location-based media demonstrate how digital systems can encourage physical exploration and engagement with geographic space. Social platforms highlight the importance of participation mechanics that encourage users to contribute rather than passively consume content. Gamification research shows how reward structures and challenges can motivate sustained interaction. Finally, tourism studies illustrate the growing influence of digital platforms in shaping how people explore and experience places.

However, most existing systems emphasize only one of these dimensions at a time. Location-based games focus on spatial exploration but often limit user creativity in shaping experiences. Social media platforms enable content sharing but rarely connect participation directly to physical environments. Gamified systems encourage engagement but are typically disconnected from real-world locations.

These gaps suggest an opportunity to design platforms that integrate location-based interaction, user-generated challenges, and gamified participation into a unified system. The SideQuest platform draws inspiration from these prior systems while seeking to create a new form of experience centered on spontaneous exploration, creativity, and social interaction within physical environments.
...
The user interface should emphasise the core loop and avoid unneccesary clutter. SideQuest is image-first and interaction-first, so the design should prioritize photos, maps, and gestures rather than long blocks of text.

\subsection{Clear Affordances}
The interface should singnal how it is meant to be used. Layered cards should feel swipeable. Map markers should feel tappable. Blocked/incomplete user actions should be clearly explained. Users should not have to guess what a screen is for or if their submission went through. 

\subsection{Do Not Make the User Wait}
One of the most important product goals is responsivness. Waiting breaks the feeling of exploration and play, so the app should avoid unnecesary delay wherever possible. This principle later justifises caching, prefetching, optimisitic uploads, and queued voting.
...
Although the original model was one challenge, one location, mamy photos, user behavior showed that some kinds of participation did not fit neatly into a fully location-locked structure. Users wanted to post similar ideas from different places, compete with friends in different locations, and create broader thematic prompts. In response, SideQuest introduced non-location-locked Quests while keeping geo-locking as the default. This preserved the original emphasis on place-based discovery while allowing the system to support a wider range of social and creative use cases.
...
A small region-specific mechanism affects map display in mainland China. When needed, stored coordinates are transformed before rendering so that map markers align with the local mapping standard. This logic is kept separate from the main content model so that the rest of the system remains unchanged.
...
\caption{Before and after for uploading a photo.}
...
The next redesign left the cards stacked on top of one another, but a swipe left or right would enlarge the card on that side to fill the whole screen. Releasing your finger at the end of the swipe would submit that photo as the winner, while leaving your finger on the display would let you keep looking at the photos. While this design allowed quick, one-movement voting, some users didn't even understand they were voting. Very few phone design elements require the finger to be pressed to the screen continuously to avoid submitting, and this movement was not intuitive to users as they just `swiped' to browse the photos just like in a photo gallary.
...
The next section will follow Kailani, one of our test users, to demonstrate how the app's features and interactions.
...
\section*{Conclusion}
SideQuest is a full-stack application following a common development lifecycle from design and development, to deployment and testing. Promising active user numbers shortly after the app's release provide evidence that the app can suceed at scale. Going forward, features will continue to be built out iterativley using user feedback, with the goal of builing out a userbase of users that delight in the app.
...
% Example: citations used above (these two exist in bibfile.bib)
% \cite{Zhang2021}
% \cite{Axler2020}
...
\begin{itemize}
  \item SideQuest iOS App: \href{https://example.com/app-store-link}{App Store link (placeholder)}
\end{itemize}

\textbf{Secondary Artifacts}
\begin{itemize}
  \item Frontend and Backend GitHub Repositories (source code, commit history): \\
  \href{https://github.com/example/frontend}{Frontend repository (placeholder)}, \\
  \href{https://github.com/example/backend}{Backend repository (placeholder)}
  \item Deployed Backend (Docker container on Google Cloud Run): \\
  \href{https://example.com/cloud-run-endpoint}{Backend service endpoint (placeholder)}
  \item Screenshots of App State
  \item Diagrams of database, tech stack, development workflow, etc.
\end{itemize}

\section{Additional Screenshots (Example)}
Put overflow UI screens, edge cases, and iterations here.

\section{System Diagrams (Example)}
Put full-resolution architecture diagrams here.
